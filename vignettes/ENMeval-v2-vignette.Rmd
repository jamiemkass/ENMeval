...
Although older versions (0.3.0 and earlier) implemented only the ENM presence-only algorithm Maxent through the [Java software](http://biodiversityinformatics.amnh.org/open_source/maxent/) and R with the package  [`maxnet`](https://cran.r-project.org/package=maxnet), this new version 2.0.0 can implement any ENM in theory provided its settings are specified in a *ENMdetails* object (see below for an example). Additional built-in algorithms included in this version are specified as *ENMdetails* objects for reference: [boosted regression trees](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/j.1365-2656.2008.01390.x), [random forest](https://www.researchgate.net/profile/Andy_Liaw/publication/228451484_Classification_and_Regression_by_RandomForest/links/53fb24cc0cf20a45497047ab/Classification-and-Regression-by-RandomForest.pdf), and  [BIOCLIM](https://onlinelibrary.wiley.com/doi/full/10.1111/ddi.12144). 
...
# For example, rasterVis needs to be loaded before ggplot2 or else layer() doesn't work, etc.
# Keep in mind that conflicts such as this can lead to otherwise mysterious errors
library(raster)
library(dismo)
library(dplyr)
...
```{r}
# You'll need to install rmaxnet from Github because it's not on CRAN
...
evalplot.envSim.hist(envs = envs, occs = occs, bg = bg, occs.grp = block$occs.grp, bg.grp = block$bg.grp, pts.type = "occs", sim.type = "mess")
evalplot.envSim.hist(envs = envs, occs = occs, bg = bg, occs.grp = block$occs.grp, bg.grp = block$bg.grp, pts.type = "occs", sim.type = "most_diff")
evalplot.envSim.hist(envs = envs, occs = occs, bg = bg, occs.grp = block$occs.grp, bg.grp = block$bg.grp, pts.type = "occs", sim.type = "most_sim")
...
evalplot.envSim.map(envs = envs, occs = occs, bg = bg, occs.grp = block$occs.grp, bg.grp = block$bg.grp, pts.type = "occs", sim.type = "mess", bb.buf = 7)
evalplot.envSim.map(envs = envs, occs = occs, bg = bg, occs.grp = block$occs.grp, bg.grp = block$bg.grp, pts.type = "occs", sim.type = "most_diff", bb.buf = 7)
evalplot.envSim.map(envs = envs, occs = occs, bg = bg, occs.grp = block$occs.grp, bg.grp = block$bg.grp, pts.type = "occs", sim.type = "most_sim", bb.buf = 7)
```
...
evalplot.envSim.hist(envs = envs, occs = occs, bg = bg, occs.grp = cb1$occs.grp, bg.grp = cb1$bg.grp, pts.type = "occs", sim.type = "mess")
evalplot.envSim.map(envs = envs, occs = occs, bg = bg, occs.grp = cb1$occs.grp, bg.grp = cb1$bg.grp, pts.type = "occs", sim.type = "mess", bb.buf = 7)
...
evalplot.envSim.hist(envs = envs, occs = occs, bg = bg, occs.grp = cb1.large$occs.grp, bg.grp = cb1.large$bg.grp, pts.type = "occs", sim.type = "mess")
evalplot.envSim.map(envs = envs, occs = occs, bg = bg, occs.grp = cb1.large$occs.grp, bg.grp = cb1.large$bg.grp, pts.type = "occs", sim.type = "mess", bb.buf = 7)
```
...
evalplot.envSim.hist(envs = envs, occs = occs, bg = bg, occs.grp = cb2$occs.grp, bg.grp = cb2$bg.grp, pts.type = "occs", sim.type = "mess")
evalplot.envSim.map(envs = envs, occs = occs, bg = bg, occs.grp = cb2$occs.grp, bg.grp = cb2$bg.grp, pts.type = "occs", sim.type = "mess", bb.buf = 7)
```
...
evalplot.envSim.hist(envs = envs, occs = occs, bg = bg, occs.grp = rand$occs.grp, bg.grp = rand$bg.grp, pts.type = "occs", sim.type = "mess")
evalplot.envSim.map(envs = envs, occs = occs, bg = bg, occs.grp = rand$occs.grp, bg.grp = rand$bg.grp, pts.type = "occs", sim.type = "mess", bb.buf = 7)
```
...
occs.test <- rbind(occs, occs.testing)
# Here we label the fully withheld test group with a partition value of 2, and the occurrences with 1
occs.test.grp <- c(rep(1, nrow(occs)), rep(2, nrow(occs.testing)))
evalplot.grps(pts = occs.test, pts.grp = occs.test.grp, envs = envs.bg)
...
evalplot.envSim.hist(envs = envs, occs = occs.test, bg = bg, occs.grp = occs.test.grp, bg.grp = rand$bg.grp, pts.type = "occs", sim.type = "mess")
evalplot.envSim.map(envs = envs, occs = occs.test, bg = bg, occs.grp = occs.test.grp, bg.grp = rand$bg.grp, pts.type = "occs", sim.type = "mess", bb.buf = 7)
```
...
e.mx.l <- ENMevaluate(occs = occs, envs = envs, bg = bg, algorithm = 'maxnet', partitions = 'checkerboard2', 
...
```{r maxnet2, eval=TRUE}
e.mx.lqh <- ENMevaluate(occs = occs, envs = envs, bg = bg, algorithm = 'maxnet', partitions = 'checkerboard2', 
                        tune.args = list(fc = c("L","LQ","LQH","H"), rm = 1:3))
```

When building many models, the command may take a long time to run. Of course this depends on the size of your dataset and the computer you are using. When working on big projects, running the command in parallel (`parallel=T`) can be considerably faster. Note that running parallel can also be slower when working on small datasets.

```{r maxnet2par, eval=FALSE}
e.mx.lqh <- ENMevaluate(occs = occs, envs = envs, bg = bg, algorithm = 'maxnet', partitions = 'checkerboard2', 
                        tune.args = list(fc = c("L","LQ","LQH","H"), rm = 1:3), parallel = TRUE)
```
...
e.swd <- ENMevaluate(occs.z, bg = bg.z, algorithm = "maxnet", tune.args = tune.args, partitions = "block", categoricals = "biome")
```
...
e.mx.proc <- ENMevaluate(occs, envs, bg, algorithm = "maxnet", tune.args = tune.args, partitions = "block", user.eval = proc)
...
```{r}
# Here, we demonstrate modifying the BRT ENMdetails object to add the bag.fraction parameter for tuning and using it as a user.enm
...
my.brt <- enm.boostedRegressionTrees
my.brt@args.train <- function(occs.z, bg.z, tune.i, other.settings) {
  out <- list()
  d <- rbind(occs.z, bg.z)
  ...
  out$silent <- TRUE
  # inserted here ############
  out$bag.fraction <- tune.i$bf
  # ##########################
  out <- c(out, other.settings$other.args)
  return(out)
}
my.brt@args.val <- function(occs.z, bg.z, tune.i, other.settings, mod.full = NULL) {
  out <- list()
  d <- rbind(occs.z, bg.z)
  p <- c(rep(1, nrow(occs.z)), rep(0, nrow(bg.z)))
  out$data <- cbind(p, d)
...
e.userENM <- ENMevaluate(occs, envs, bg, user.enm = my.brt, tune.args = list(tc = 1:2, lr = c(0.01,0.001), bf = c(0.5, 0.75)), partitions = "block")
```
