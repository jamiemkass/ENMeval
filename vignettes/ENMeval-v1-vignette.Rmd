...
eval.indPart <- ENMevaluate(occs, envs, bg, mod.name = "maxnet", 
...
eval.userENM <- ENMevaluate(occs, envs, bg, user.enm = enm.maxnet, 
                             tune.args = tune.args, categoricals = "biome", partitions = "block")
# You will see the results of the new statistics in the results table
eval.userENM@results
```
...
eval.userENM <- ENMevaluate(occs, envs, bg, user.enm = enm.myMod, 
                             tune.args = tune.args, categoricals = "biome", partitions = "block")
```
...
eval.algorithm(eval)
eval.tune.settings(eval)
eval.partition.method(eval)
...
#### Metadata {#metadata}
ENMeval now catalogs details of the analysis essential for reproducibility in a rangeModelMetadata list, accessible from the ENMevaluation object output. The framework and philosophy behind rangeModelMetadata is described in [Merow et al. 2019](https://doi.org/10.1111/geb.12993). 
