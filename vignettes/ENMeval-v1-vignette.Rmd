...
cbi: a) calculated on raster model prediction based on `envs`, b) cbi.val cannot only be calculated for partitions "randomkfold", "testing", or "user"
...
e.test <- ENMevaluate(occs, envs, bg, mod.name = "maxnet", 
...
The last way to run `ENMevaluate()` is by specifying user model settings or completely different models. Users can either modify how built-in models function, or specify their own. This is all done using the `ENMdetails` object. Built-in models (maxent.jar, maxnet, BIOCLIM, boostedRegressionTrees) are implemented as `ENMdetails` objects. You can find them in the /R folder with the prefix "enm", as in "enm.maxnet". The `ENMdetails` object specifies the way `ENMevaluate()` should run the model. It defines the model's name, what the model function is, how to assemble arguments for the model function, how (or if) to calculate AIC, how to run `dismo::evaluate()` for evaluation statistics, which additional evaluation statistics to calculate, how to make model predictions, how to extract the number of model parameters, and particular messages to write to the console during the analysis.
...
# The e.val parameter is the ModelEvaluation object (from dismo::evaluate()) calculated on the test data
# See the documentation in ?enm.maxnet for more details on the input parameters
enm.maxnet@kstats <- function(e.val, mod, other.args) {
  user.kstats <- c(maxTSS.val = max(e.val@TPR + e.val@TNR) - 1, maxKappa.val = max(e.val@kappa))
  return(user.kstats)
}
