...
library(rmaxent)
library(rasterVis)
# First we extract the climatic variable values at the occurrence points -- these values
# are our "reference"
...
evalplot.grps(pts = occs, pts.grp = block$occ.grp, envs = envs.bg)
...
evalplot.grps(pts = occs, pts.grp = cb1$occ.grp, envs = envs.bg)
...
evalplot.grps(pts = occs, pts.grp = cb1.large$occ.grp, envs = envs.bg)
...
evalplot.grps(pts = occs, pts.grp = cb2$occ.grp, envs = envs.bg)
# Plotting the background points shows the checkerboard pattern very clearly
...
evalplot.grps(pts = occs, pts.grp = jack$occ.grp, envs = envs.bg)
```
...
evalplot.grps(pts = occs, pts.grp = occ.grp, envs = envs.bg)
```
...
# the size of blocks in meters with the parameter theRange (here set at 1000 km), and the 
...
e.mx.l <- ENMevaluate(occ = occs, env = envs, bg.coords = bg, mod.name = 'maxnet',
...
```{r maxnet2, eval=TRUE}
e.mx.lqh <- ENMevaluate(occ = occs, env = envs, bg.coords = bg, mod.name = 'maxnet',
...
e.mx.lqh.nr <- ENMevaluate(occ = occs, env = envs, bg.coords = bg, mod.name = 'maxnet',
                             partitions = 'checkerboard2', tune.args = list(fc = c("L","LQ","LQH","H"), rm = 1:3),
                             rasterPreds = NULL)
```
...
e.mx <- ENMevaluate(occs, envs, bg, mod.name = "maxnet", 
...
e.mxjar <- ENMevaluate(occs, envs, bg, mod.name = "maxent.jar", 
                    tune.args = tune.args, categoricals = "biome", partitions = "block")
...
e.ind <- ENMevaluate(occs, envs, bg, mod.name = "maxnet", 
...
e.user <- ENMevaluate(occs, envs, bg, mod.name = "maxnet", 
...
e.noCV <- ENMevaluate(occs, envs, bg, mod.name = "maxnet", 
...
e.swd <- ENMevaluate(occs.vals, bg = bg.vals, mod.name = "maxnet", 
                        tune.args = tune.args, categoricals = "biome", partitions = "block")
...
e.bgCBI <- ENMevaluate(occs, envs, bg, mod.name = "maxnet", 
                          tune.args = tune.args, categoricals = "biome", partitions = "block", 
                          cbi.eval = "bg")
```

The last way to run `ENMevaluate()` is by specifying user model settings or completely different models. Users can either modify how built-in models function, or specify their own. This is all done using the `ENMdetails` object. Built-in models (maxent.jar, maxnet, BIOCLIM, brt) are implemented as `ENMdetails` objects. You can find them in the /R folder with the prefix "enm", as in "enm.maxnet". The `ENMdetails` object specifies the way `ENMevaluate()` should run the model. It defines the model's name, what the model function is, how to assemble arguments for the model function, how (or if) to calculate AIC, how to run `dismo::evaluate()` for evaluation statistics, which additional evaluation statistics to calculate, how to make model predictions, how to extract the number of model parameters, and particular messages to write to the console during the analysis.

Users can modify the built-in `ENMdetails` objects to customize an analysis. For example, the user can redefine the "kstats" slot of an existing `ENMdetails` objects to calculate the average max TSS and max Kappa statistics for testing data. In order to run the model with this new behavior, the user needs to input the modified `ENMdetails` object in as the "user.enm" parameter. In this case, "mod.name" is left at the default NULL.

```{r}
# This is the existing ENMdetails object for maxnet
enm.maxnet
# This is the existing kstats function for maxnet
enm.maxnet@kstats
# This kstats function can be redefined
# The e.test parameter is the ModelEvaluation object (from dismo::evaluate()) calculated on the test data
# See the documentation in ?enm.maxnet for more details on the input parameters
enm.maxnet@kstats <- function(e.test, mod, other.args) {
  user.kstats <- c(maxTSS.test = max(e.test@TPR + e.test@TNR) - 1, maxKappa.test = max(e.test@kappa))
  return(user.kstats)
}
# ENMevaluate() can now be run with the modified ENMdetails object
e.userENM1 <- ENMevaluate(occs, envs, bg, user.enm = enm.maxnet, 
                             tune.args = tune.args, categoricals = "biome", partitions = "block")
# You will see the results of the new statistics in the results table
e.userENM1@results
```

Users can also construct their own `ENMdetails` object using the built-in ones as guides. For example, a user can copy the "enm.maxnet.R" script, modify the code to specify a different model and save as a new script in the /R folder, and use it to run `ENMevaluate()`.

```{r}
# If you saved a new script for "enm.myMod.R" and specified all the internal functions, 
# run ENMevaluate() like this
# NOTE: It is important to make sure that your custom model can handle categorical variables
# By default, ENMevaluate() removes them for BIOCLIM models, but if you specify a custom model,
# you will need to remove them yourself before running ENMevaluate()
e.userENM2 <- ENMevaluate(occs, envs, bg, user.enm = enm.myMod, 
                             tune.args = tune.args, categoricals = "biome", partitions = "block")
```
...
points(occs(e.mx), pch = 21, bg = occ.grp(e.mx))
```
...
mod.null <- ENMnullSims(eval, mod.settings = list(fc = "L", rm = 2), no.iter = 10, 
                     envs = envs, userStats.signs = list(maxTSS.test = 1, maxKappa.test = 1))
null.results(mod.null)
null.results.grp(mod.null)
...
rmm$model$selectionRules <- "lowest 10 percentile omission rate, break ties with average test AUC"
