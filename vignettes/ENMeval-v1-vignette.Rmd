...
cbi: a) calculated on raster model prediction based on `envs`, b) cbi.test cannot only be calculated for partitions "randomkfold", "testing", or "user"
...
e.testing <- ENMevaluate(occs, envs, bg, mod.name = "maxnet", 
                        tune.args = tune.args, categoricals = "biome", partitions = "testing", 
...
# 5. No raster data (a.k.a, samples with data, or SWD): no full model raster predictions created, so will run faster; also, both cbi.train and cbi.test will be calculated on the point data (training and validation background) instead of on the "envs" rasters (default)
occs.z <- cbind(occs, raster::extract(envs, occs))
bg.z <- cbind(bg, raster::extract(envs, bg))
e.swd <- ENMevaluate(occs.z, bg = bg.z, mod.name = "maxnet", 
                        tune.args = tune.args, categoricals = "biome", partitions = "block")
```
...
Users can modify the built-in `ENMdetails` objects to customize an analysis. For example, the user can redefine the "kstats" slot of an existing `ENMdetails` objects to calculate the average max TSS and max Kappa statistics for validation data. In order to run the model with this new behavior, the user needs to input the modified `ENMdetails` object in as the "user.enm" parameter. In this case, "mod.name" is left at the default NULL.
