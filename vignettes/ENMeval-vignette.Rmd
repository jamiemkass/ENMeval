...
- [Partitioning Occurrences for Evaluation](Partitioning Occurrences for Evaluation)
- [Running ENMeval](Running ENMeval)
...
# Plot all the occurrence points (notice the stray point in the ocean, we'll take care of that later...)
points(occs)
```
...
# Add 1 degree to each bound (buffer)
bb.buf <- extent(bb[1]-1, bb[3]+1, bb[2]-1, bb[4]+1)
...
# Both spatial objects have same geographic coordinate system, so just name the CRS for occs with that of wrld_simpl occs.sp@proj4string <- crs(ca.sa)  

# Run a spatial intersect operation to subset occs to only the points that fall within our polygon
occs.sp <- occs.sp[ca.sa]

# Rename occs with the new subset
occs <- occs.sp@coords  

# Mask envs by this polygon
envs.backg <- mask(envs.backg, ca.sa)  

# This is our background extent with occurrence records
plot(envs.backg[[1]])
points(occs)
```
...
# NOTE: Since the raster has < 10,000 pixels, you will get a warning message and all pixels will be used for background.
plot(envs.backg[[1]])  
...
plot(env2@predictions[[which(eval2@results$delta.AICc==0]])
