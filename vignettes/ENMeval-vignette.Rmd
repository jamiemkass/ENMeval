...
# The "lambdas" slot shows which variables were included in the model. After the variable name, the next number is the variable coefficient, then the min and max of that variable for the inut data. If the coefficient is 0, that variable was not included in the model.
opt@lambdas

# The "results" slot shows the Maxent model statistics:
opt@results

# You can use these functions to get sorted data frames of permutation importance and percent contribution.
permImp(opt)
pntContr(opt)
...
pi <- permImp(opt)
barplot(pi$permImp, names.arg=pi$variable, las=2, ylab="Permutation Importance")
```
...
## Downstream Analyses {#downstream}
###**UNDER CONSTRUCTION**
Below is a running list of other things we are planning to add to this vignette.  Feel free to let us know if there are particular things you would like to see added.
...
###### Parse text of lambda file
In order to determine which variables were used for each model, you can look into the lambda file generated by maxent.  This file is stored in each maxent model object in a slot called 'lambdas'.

```{r parse.lambdas}
# Let's look at the lambda file for the model object of our "AICc optimal" model:
eval2@models[[which(eval2@results$delta.AICc==0)]]@lambdas
```

This returns a character vector where all entries except the last four lines give the model coefficient for each variable used.  You will probably find the syntax to be cryptic and the information is not stored in a very user-friendly way.  Fortunately, John Baumgartner has developed a useful function to parse this file into a more user-friendly data.frame.  See the `parse_lambdas.R` function in his [`rmaxent` package] (https://github.com/johnbaums/rmaxent/blob/master/R/parse_lambdas.R) for more details.
