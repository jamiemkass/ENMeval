...
```{r occDownload}
install.packages('spocc') #####NOT sure where you guys prefer to put these, but for now, I will add above the library command. Should we specify a repo?
library(spocc)
...
# NOTE: this will create a warning message since the raster has less than 10,000 pixels. All pixels will be used for background.
plot(envs.backg[[1]])  
...
res <- eval2@results  # Model evaluation statistics are available if we isolate the results table 
...
# access a RasterStack of the model predictions. NOTE: these predictions are in the raw output format.
eval2@predictions

# let's plot the optimal model according to AICc (model with delta AICc equal to 0). Notice that this model was the 4th one that we ran (row 4), which means that it will be the 4th item in our RasterStack
plot(env2@predictions[[4]])
# gives us the same raster as:
plot(eval2@predictions[[as.numeric(rownames(res[res$delta.AICc==0,]))]])
```
