...
We are going to model the climatic niche suitability for our focal species using climate data from [WorldClim](http://www.worldclim.org/). WorldClim has a range of variables available at various resolutions; for simplicity, here we'll use the 9 bioclimatic variables at 10 arcmin resolution (about 20 km across at the equator) included in the `predicts` package from WorldClim 1.4. These climatic data are based on 50-year averages from 1950-2000.

```{r envDownload, warning = FALSE, message = FALSE}
# Locate the predictor raster files from the predicts folder.
envs.files <- list.files(path = paste(system.file(package='predicts'), '/ex', sep = ''),
                         pattern = 'tif$', full.names = TRUE)

# Read the raster files into a SpatRaster.
# These variables represent 9 bioclimatic variables and one categorical variable "biome".
# We will remove the categorical variable for this vignette for simplification,
# but ENMeval provides functionality for categorical variables (see documentation).
# Find the descriptions of the bioclimatic variables here: 
# https://www.worldclim.org/data/bioclim.html
envs <- terra::rast(envs.files)
# Remove the "bipme" variable for this demonstration. This biome variable has
# NA values for cells that have values for the climatic variables, so if you
# decide to use it, make sure to mask the raster stack to it before analysis.
# Note that ENMevaluate will do this internally anyway.
# Also, if you use categorical variables, make sure to declare them as factors
# with terra::as.factor.
envs$biome <- NULL
...
pr <- predicts::partialResponse(e.mxjar@models[[as.vector(opt.seq$tune.args)]],
                                var = "bio5")
...
# model object above.
pred.seq <- eval.predictions(e.mx)[[as.vector(opt.seq$tune.args)]]
...
# example, the empirical validation AUC value (solid red line) is significantly 
