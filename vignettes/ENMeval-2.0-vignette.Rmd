...
options(terra.pal = terra::map.pal("plasma", 100))
```
...
* To align with the new `ENMeval` 2.0.6, this version updates the vignette by replacing `raster` functions with those from `terra`, and `dismo` functions with those from `predicts`. These updates were made by G. E. Pinilla-Buitrago.
...
And these are the marginal response curves for the predictor variables with non-zero coefficients in our model, plotted with `evalplot.respCurves()` and `evalplot.respCurve()`. We define the y-axis to be the cloglog transformation, which is an approximation of occurrence probability (with assumptions) bounded by 0 and 1 (Phillips et al. 2017). Unlike `plot()` for `maxnet` models or `predicts::partialResponse()` for `maxent.jar` models, these functions work identically regardless of which algorithm was used to fit the model. One difference to note: `plot()` always holds the non-focal variables at the model's internal, occurrence-only sample means, while `evalplot.respCurves()`/`evalplot.respCurve()` hold them at the mean (or other summary function, via the `fun` argument) of whichever `data` you provide. Below we use occurrence data only to match `plot()`'s behavior, but background data could be included instead for a different reference point.
...
The response curves in ENMeval also allow you to control how you plot the curves via two types. The default, `type = 1`, holds all other variables at their mean (can be changed whit the fun parameter) while the focal variable varies across its range, as used in `maxnet`'s `plot()` and the maxent.jar/dismo response curves use. `type = 2` instead holds the focal variable at each grid value while keeping every other variable at its own observed value, as used by the `pdp` and `predicts` packages. It takes longer and consumes more of your computer's resources. The two can diverge, as shown here for `bio1`:
...
curve1 <- evalplot.respCurve(mod.complex, data, var = "bio1", type = 1)
curve2 <- evalplot.respCurve(mod.complex, data, var = "bio1", type = 2)
...
ggplot(rbind(d1, d2), aes(x = x, y = y, color = type)) +
  ...
  ylim(c(0, 1)) +
  xlab("bio1") + ylab("predicted suitability") +
  theme_classic()
```
