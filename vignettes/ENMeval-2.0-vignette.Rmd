...
# We change back from a RasterBrick to SpatRaster because of issues with assigning 
# factor rasters for RasterBricks.
envs <- terra::mask(envs, envs[[10]])
# Make sure to declare the categorical variable as a factor
envs$biome <- terra::as.factor(envs$biome)
...
occs.z <- terra::extract(envs[[-10]], occs, ID = FALSE)
...
# points.
occs.sim <- ENMeval::similarity(envs[[-10]], occs.z)
...
myScale <- seq(terra::minmax(occs.mess)[1], terra::minmax(occs.mess)[2], length.out = 100)
rasterVis::levelplot(occs.mess, main = "Environmental similarity", at = myScale, margin = FALSE) + 
  latticeExtra::layer(sp::sp.points(occs.sp, col="black"))
...
bg <- predicts::backgroundSample(envs.bg[[10]], n = 10000) |> as.data.frame()
