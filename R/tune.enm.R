...
cv.enm <- function(occs.vals, bg.vals, occs.folds, bg.folds, envs, mod.fun, 
  ...
  auc.train <- dismo::evaluate(occs.vals, bg.vals, mod.full)@auc
...
collateResults <- function(results, tune.tbl) {
  ...
  nparams <- sapply(mod.full.all, function(x) getNoParams(x, mod.fun.name))
